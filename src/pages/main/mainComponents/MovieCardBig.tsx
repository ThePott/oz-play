...
const MovieCardBig = ({ movieCardInfo }: { movieCardInfo: MovieCardInfo }) => {
  ...
  return (
    <Link to={`/detail/${movieCardInfo.id}`}
      className={`transition 
        w-[400px] shrink-0
        h-[600px] overflow-hidden bg-zinc-800 flex flex-col items-start relative rounded-xl`}
      onMouseEnter={() => setIsMouseOver(true)}
      onMouseLeave={() => setIsMouseOver(false)}>

      <img src={posterSrc} alt={`${movieCardInfo.title}__poster`} className={`transition ${isMouseOver && "scale-105"} duration-500 w-full`} />
