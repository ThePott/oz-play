...
const MovieCardHorizontal = ({ movieCardInfo }: { movieCardInfo: MovieCardInfo }) => {
    const posterSrc = `${imageBaseUrl}${movieCardInfo.poster_path}`
    const [isMouseOver, setIsMouseOver] = useState(false)

    return (
        <Link to={`/detail/${movieCardInfo.id}`}
            className={`min-w-[400px] transition 
        ${isMouseOver && "scale-110 z-10 -translate-y-[15px]"}
        h-[200px] overflow-hidden bg-zinc-800 flex flex-col items-start relative rounded-xl`}
